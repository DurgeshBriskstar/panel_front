...
export function imageHandler() {
    ...
    input.onchange = () => {
        ...
        if (file != null && file.type ==='image/jpg' || file.type ==='image/jpeg' || file.type ==='image/png' || file.type ==='image/gif' || file.type ==='image/svg' || file.type ==='image/svg+xml') {
            // image upload on server 
            dispatch(uploadTempImage(file)).then((res) => {
                const range = this.quill.getSelection(true);
                this.quill.insertEmbed(range.index, 'image', res.data, 'user');
            })
        }
    };
}
